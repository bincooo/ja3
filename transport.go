...
func NewTransport(args ...TransportArgs) http.RoundTripper {
	...
	dialTLSContext := func(ctx context.Context, network, addr string) (net.Conn, error) {
		...
		if proxies := tripper.getProxy("tls"); proxies != "" {
			var err error
			dialConn, err = newConn(ctx, proxies, addr)
			if err != nil {
				return nil, err
			}
		}
	...
	tripper.originalTransport.DialTLSContext = dialTLSContext
...
func newConn(ctx context.Context, proxies, addr string) (dialConn net.Conn, err error) {
	...
	case "http", "https":
		var d proxy.Dialer
		d, err = NewWithConfig(u, proxy.Direct, &Config{Context: ctx})
		if err != nil {
